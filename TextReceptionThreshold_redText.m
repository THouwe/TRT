function TextReceptionThreshold_redText(varargin)
...
% - ptMasking [integer]: percent masking. Default: 50;
...
% - setSeed [integer]: for randomizing grating starting location of masker bars
%       (for reproducibility). Default: random integer between 1 and nOffsets.
...
totAreaToCover=thinRectArea*options.ptMasking/100;
...
%         window, options.textString, 'center', 'center', white,[],[],[],[],[],centeredRect);
    [tx, ty, targetTextBounds, targetWordBounds] = DrawFormattedText(...
        window, options.textString, 'center', 'center', [255 0 0],[],[],[],[],[],centeredRect);
end
...
if options.closeAfterNsec > 0
    WaitSecs(options.closeAfterNsec)
    Screen('Close', window);
end
